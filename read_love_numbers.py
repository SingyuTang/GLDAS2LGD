...
# 根据相对路径获取当前文件所在包内的绝对路径
def get_data_path(relpath: Union[list, str, pathlib.Path]):
    """
     根据相对路径获取当前文件所在包内的绝对路径。

    该函数接受相对路径作为参数，该路径可以是字符串、列表或Path对象。
    如果路径是列表，则会将列表中的每个元素视为路径的一部分进行拼接。
    如果路径是字符串，则直接将其视为相对路径。
    然后返回当前文件所在目录与相对路径拼接后的绝对路径。
    Parameters
    ----------
    relpath: list, str or pathlib.Path
        相对路径，可以是单个路径字符串，也可以是路径元素列表。
    Returns
    -------
    pathlib.Path
        拼接后的绝对路径对象。
